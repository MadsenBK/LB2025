...
export type Subjects = "all" | "sa_dashboard" | "oa_dashboard" | "os_dashboard" | "organization-setup" | "setup_error" | "post" | "user" | "client"; // ✅ Added "client"
...
export default function defineAbilitiesFor(role: string) {
  ...
  if (role === "sys_admin") {
    ...
    can("manage", "client"); // ✅ Org admin can manage clients
    ...
    can("read", "profile");
    can("manage", "client"); // ✅ Org staff can also manage clients
  } else {
    can("read", "login");
    can("read", "/");
  }
